...
\openingrep{
选题目的\\
如何高效存储大量字符串以及快速检索其中的近似子串，在基因突变序列、Wiki历史条目等问题中起着关键作用。目前较多的方法只侧重于其中一点，即通过索引进行压缩，但同时失去了检索的可能。因此，找出一种兼备存储效率及检索速度的方法很有实用意义。\\
\\
思路及方法\\
对于文本压缩，比较常见的方法是对于某些近似的字符串，选出一个字符串作为它们的索引。该方法的问题在于，对于需要检索的字符串，只能找出与其近似的索引，而不是全部近似子串的集合。
在这个方法的基础上加以改进，我们可以构建分层结构的多重引用，即每个字符串与索引间存在层次化的依赖关系。对于检索，需要先找到匹配的索引，通过对索引中预处理信息的考察，得出所有与模式串编辑距离小于k的子串集合。
问题的难点在于，找出合适的算法与数据结构，用于构造字符串间的引用关系，在保证较高压缩率的同时，兼顾检索的速度。\\
\\
进度安排\\
...
%第一次过程检查的学生总结
\firstcheck{\rule{0cm}{4\baselineskip}}
%第一次过程检查的指导老师意见
\firstopinion{\rule{0cm}{2\baselineskip}}
%第二次过程检查的学生总结
\secondcheck{\rule{0cm}{4\baselineskip}}
%第二次过程检查的指导老师意见
\secondopinion{\rule{0cm}{2\baselineskip}}
%第三次过程检查的学生总结
\thirdcheck{\rule{0cm}{4\baselineskip}}
%第三次过程检查的指导老师意见
...
\ekeywords{typeset; thesis; template}
